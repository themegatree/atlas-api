        ...
        cy.fixture('/csv-files/SelfAssessments/malformed.csv').then(fileContent => {
          ...
          cy
            ...
            .should(res => {
              ...
              const text = enc.decode(res.body)
              const parsedData = JSON.parse(text)
              expect(parsedData.response[0]).to.eq(`Headers: [confidenceScore,overallScore,studentReason,studentFeedback,submissionsDate,dueDate] do not match any valid headers.`)
            })
        })
      })
        ...
        cy.fixture('/csv-files/SelfAssessments/empty.csv').then(fileContent => {
          ...
          cy
            ...
            .should(res => {
              ...
              const text = enc.decode(res.body)
              const parsedData = JSON.parse(text)
              expect(parsedData.response[0]).to.eq(`Headers: [65,1,2,No reason,No Feedback,08/17/21 14:35,08/17/21 14:35] does not match Headers for Self Assessment: [StudentId,confidenceScore,overallScore,studentReason,studentFeedback,dueDate,submissionDate]`)
            })
        })
      })
