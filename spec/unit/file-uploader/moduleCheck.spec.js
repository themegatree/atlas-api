...
describe("ModuleCheck functions ", () => {
  ...
  it("checking the projectCheck method works with errors", async () => {
    ...
    expect(moduleChallengeChecker.errors).toEqual([ "You have entered an incorrect language on line 6" ]);
  });
  ...
  it("checking the scoreCheck method works with errors", async () => {
    ...
    expect(moduleChallengeChecker.errors).toEqual([ "Coach Score: finished is invalid, on line 6" ]);
  });
});